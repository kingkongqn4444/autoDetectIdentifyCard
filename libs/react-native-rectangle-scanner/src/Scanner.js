...
class Scanner extends React.Component {
  static propTypes = {
    ...
    androidPermission: PropTypes.oneOfType([
      PropTypes.bool,
      PropTypes.shape({
  ...
  componentWillUnmount() {
    if (CameraManager.cleanup) {
      CameraManager.cleanup();
    }
  }

  getImageQuality() {
    if (!this.props.capturedQuality) {
      return 0.8;
    }
    if (this.props.capturedQuality > 1) {
      return 1;
    }
    if (this.props.capturedQuality < 0.1) {
      return 0.1;
    }
  ...
  sendOnPictureProcessedEvent = event => {
    if (!this.props.onPictureProcessed) {
      return null;
    }
  ...
  sendOnErrorProcessingImage = event => {
    if (!this.props.onErrorProcessingImage) {
      return null;
    }
  ...
  sendOnRectangleDetectedEvent = event => {
    if (!this.props.onRectangleDetected) {
      return null;
    }
