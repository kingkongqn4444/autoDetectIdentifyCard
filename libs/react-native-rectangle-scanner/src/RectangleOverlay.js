...
function getDifferenceBetweenRectangles(firstRectangle, secondRectangle) {
  const topRightXDiff = Math.abs(
    firstRectangle.topRight.x - secondRectangle.topRight.x,
  );
  const topRightYDiff = Math.abs(
    firstRectangle.topRight.y - secondRectangle.topRight.y,
  );

  const topLeftXDiff = Math.abs(
    firstRectangle.topLeft.x - secondRectangle.topLeft.x,
  );
  const topLeftYDiff = Math.abs(
    firstRectangle.topLeft.y - secondRectangle.topLeft.y,
  );

  const bottomRightXDiff = Math.abs(
    firstRectangle.bottomRight.x - secondRectangle.bottomRight.x,
  );
  const bottomRightYDiff = Math.abs(
    firstRectangle.bottomRight.y - secondRectangle.bottomRight.y,
  );

  const bottomLeftXDiff = Math.abs(
    firstRectangle.bottomLeft.x - secondRectangle.bottomLeft.x,
  );
  const bottomLeftYDiff = Math.abs(
...
function detectCMND(length, width) {
  const size = length / width;
  // doc
  if (length > width && length < 900) {
    if (size > 1.43 && size < 1.5) {
      return true;
    }
    return false;
  } else {
    if (length < 500) {
      if (size > 0.7 && size < 0.75) {
        return true;
      }
    }
    return false;
...
export default class RectangleOverlay extends Component {
  ...
  static getDerivedStateFromProps(newProps, oldState) {
    if (
      newProps.allowDetection &&
      newProps.detectedRectangle &&
      oldState.lastRectangle
    ) {
      ...
      if (diff > newProps.rectangleDifferenceAllowance) {
        detectionCount = 0;
      }
      return {
  ...
  componentDidUpdate() {
    if (!this.props.onDetectedCapture) {
      return;
    }
    if (!this.foundRectangle(this.props.detectionCountBeforeCapture)) {
      return;
    }
    this.setState({
      lastRectangle: null,
    ...
    const { topLeft, bottomRight, bottomLeft } = this.props.detectedRectangle;
    var length = distance(
      bottomLeft.x,
      bottomLeft.y,
      bottomRight.x,
    ...
    var width = distance(bottomLeft.x, bottomLeft.y, topLeft.x, topLeft.y);
    if (detectCMND(length, width)) {
      this.props.onDetectedCapture();
    }
  }

  foundRectangle(detectionCount) {
    if (this.state.detectionCount < detectionCount) {
      return false;
    }
    return true;
  }
  ...
  render() {
    ...
    const { topRight, topLeft, bottomRight, bottomLeft, dimensions } =
      detectedRectangle;
